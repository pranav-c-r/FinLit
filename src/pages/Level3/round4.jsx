...
const Round4 = () => {
  ...
  const handleContinue = () => {
    ...
    setShowOptions(true);
    setMascotDialogues([
      { text: "Your body is sending signals – back pain, diabetes risk, cholesterol issues. Time to invest in health seriously or pay later!", audioSrc: null }
    ]);
  ...
  const saveUserProgress = async (roundOutcome) => {
    ...
    try {
      ...
      await updateDoc(userProgressRef, {
        'level3.round4_completed': true,
        'level3.round4': roundOutcome,
        'level': 3,
        'round': 4,
      }, { merge: true });
  ...
  const handleOptionSelection = (optionType) => {
    ...
    switch (optionType) {
      case 'Premium Health Package':
        selectedCost = '₹2L/year';
        selectedRisk = 'Low';
        selectedGrowth = 'Preventive care, early detection, quality treatment';
        selectedConsequence = 'High cost now, minimal medical emergencies later';
        miniEventImpact = 'Early detection, managed well, minimal impact';
        feedbackDialogue = 'A proactive choice! Investing in health now can save a lot of trouble and money later.';
        break;
      case 'Basic Health Coverage':
        selectedCost = '₹80K/year';
        selectedRisk = 'Medium';
        selectedGrowth = 'Adequate coverage for major issues';
        selectedConsequence = 'Moderate cost, some out-of-pocket expenses';
        miniEventImpact = 'Treatment covered, some lifestyle changes needed';
        feedbackDialogue = 'A balanced approach. Decent coverage for major issues, but be mindful of potential out-of-pocket costs.';
        break;
      case 'Minimal Health Investment':
        selectedCost = '₹30K/year';
        selectedRisk = 'High';
        selectedGrowth = 'Emergency coverage only';
        selectedConsequence = 'Low cost now, high risk of major expenses later';
        miniEventImpact = 'Major lifestyle overhaul, moderate expenses';
        feedbackDialogue = 'A risky choice. While saving money now, you are exposed to significant financial risk in case of serious illness.';
        break;
      case 'Self-Insurance Strategy':
        selectedCost = '₹3L/year saved in health fund';
        selectedRisk = 'Huge risk if major illness hits';
        selectedGrowth = 'Money grows if healthy, available for any use';
        selectedConsequence = 'Huge risk if major illness hits';
        miniEventImpact = 'Huge medical bills eat into savings';
        feedbackDialogue = 'A bold and potentially dangerous path. Self-insurance can pay off if you stay healthy, but a major illness could be catastrophic.';
        break;
      default:
        break;
    }
    ...
    setMascotDialogues([
      ...
      { text: `Mini Event – Lifestyle Disease Diagnosis at 43: ${miniEventImpact}`, audioSrc: null }
    ]);
    setCurrentDialogueIndex(0);
          ...
          <div className="bg-[#1e293b] p-4 border-b border-[#374151]">
            <div className="max-w-4xl mx-auto">
              ...
              <h1 className="text-xl md:text-2xl font-bold text-center text-white">
                {showOptions && "Age 42–43 – Health Reality & Insurance Strategy"}
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {showOptions && (
                <div className="space-y-4">
                  {/* Option 1: Premium Health Package */}
                  <div 
                    ...
                    onClick={() => handleOptionSelection('Premium Health Package')}
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-white mb-1">Premium Health Package</h3>
                        <p className="text-[#58cc02] font-bold text-xl">Cost: ₹2L/year</p>
                      </div>
                      <div className="text-3xl">🌟</div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-300">
                      <p>Risk: Low</p>
                      <p>Growth: Preventive care, early detection, quality treatment</p>
                      <p className="col-span-2">Consequence: High cost now, minimal medical emergencies later</p>
                    </div>
                  </div>
                  
                  {/* Option 2: Basic Health Coverage */}
                  <div 
                    className="bg-gradient-to-r from-[#1e293b] to-[#2d3748] p-5 rounded-2xl border-2 border-[#374151] hover:border-[#58cc02] active:scale-95 transition-all cursor-pointer shadow-lg hover:shadow-xl"
                    onClick={() => handleOptionSelection('Basic Health Coverage')}
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-white mb-1">Basic Health Coverage</h3>
                        <p className="text-orange-400 font-bold text-xl">Cost: ₹80K/year</p>
                      </div>
                      <div className="text-3xl">🏥</div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-300">
                      <p>Risk: Medium</p>
                      <p>Growth: Adequate coverage for major issues</p>
                      <p className="col-span-2">Consequence: Moderate cost, some out-of-pocket expenses</p>
                    </div>
                  </div>

                  {/* Option 3: Minimal Health Investment */}
                  <div 
                    className="bg-gradient-to-r from-[#1e293b] to-[#2d3748] p-5 rounded-2xl border-2 border-[#374151] hover:border-[#58cc02] active:scale-95 transition-all cursor-pointer shadow-lg hover:shadow-xl"
                    onClick={() => handleOptionSelection('Minimal Health Investment')}
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-white mb-1">Minimal Health Investment</h3>
                        <p className="text-red-500 font-bold text-xl">Cost: ₹30K/year</p>
                      </div>
                      <div className="text-3xl">💊</div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-300">
                      <p>Risk: High</p>
                      <p>Growth: Emergency coverage only</p>
                      <p className="col-span-2">Consequence: Low cost now, high risk of major expenses later</p>
                    </div>
                  </div>

                  {/* Option 4: Self-Insurance Strategy */}
                  <div 
                    className="bg-gradient-to-r from-[#1e293b] to-[#2d3748] p-5 rounded-2xl border-2 border-[#374151] hover:border-[#58cc02] active:scale-95 transition-all cursor-pointer shadow-lg hover:shadow-xl"
                    onClick={() => handleOptionSelection('Self-Insurance Strategy')}
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-white mb-1">Self-Insurance Strategy</h3>
                        <p className="text-purple-400 font-bold text-xl">Cost: ₹3L/year saved in health fund</p>
                      </div>
                      <div className="text-3xl">🏦</div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-300">
                      <p>Risk: Huge risk if major illness hits</p>
                      <p>Growth: Money grows if healthy, available for any use</p>
                      <p className="col-span-2">Consequence: Huge risk if major illness hits</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {showSummary && (
                <div className="space-y-6">
                  <div className="text-center py-6">
                    ...
                    <p className="text-gray-300">You've made crucial decisions about your health and insurance.</p>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-[#1e293b] to-[#2d3748] p-6 rounded-2xl border-2 border-[#374151] shadow-xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="bg-white/5 p-4 rounded-xl">
                        ...
                        <p className="font-bold text-white text-lg">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            ...
                            userChoices.risk === 'High' || userChoices.risk === 'Huge risk if major illness hits' ? 'bg-red-500/20 text-red-400' :
                      ...
                      <div className="bg-white/5 p-4 rounded-xl col-span-2">
                        <p className="text-gray-400 text-sm mb-1">Mini Event Impact (Lifestyle Disease Diagnosis at 43)</p>
                  ...
                  <div className="bg-gradient-to-r from-[#58cc02]/10 to-[#2fa946]/10 p-6 rounded-2xl border-2 border-[#58cc02]/30">
                    ...
                    <p className="text-gray-300 leading-relaxed">
                      "Prioritizing health and adequate insurance coverage becomes increasingly important as you age. Preventive care and financial planning for health emergencies are key to long-term well-being."
                    </p>
                  </div>
                  ...
                  <div className="pt-4">
                    <button 
                      ...
                      onClick={() => {
                        window.location.href = '/level3/round5';
                      }}
                    >
