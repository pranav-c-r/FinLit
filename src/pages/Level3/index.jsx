...
const Level3Overview = () => {
    ...
    const fetchRoundProgress = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setRoundProgress({
            round1_completed: data.level3?.round1_completed || false,
            round2_completed: data.level3?.round2_completed || false,
            round3_completed: data.level3?.round3_completed || false,
            round4_completed: data.level3?.round4_completed || false,
            round5_completed: data.level3?.round5_completed || false,
          });
        } else {
          console.log("No user progress found for Level 3, starting fresh.");
        }
      } catch (err) {
        console.error("Error fetching Level 3 round progress:", err);
  ...
  const renderRoundStone = (roundNumber, title) => {
    ...
    return (
      <Link 
        key={roundNumber}
        to={isUnlocked ? `/level3/round${roundNumber}` : '#'}
  ...
  const handleRestartLevel = async () => {
    ...
    if (window.confirm("Are you sure you want to restart Level 3? All your progress for this level will be reset.")) {
      try {
        const userProgressRef = doc(database, "Users", auth.currentUser.uid);
        await updateDoc(userProgressRef, {
          'level3.round1_completed': false,
          'level3.round2_completed': false,
          'level3.round3_completed': false,
          'level3.round4_completed': false,
          'level3.round5_completed': false,
          "level3.round1": null, // Reset round-specific data
          "level3.round2": null,
          "level3.round3": null,
          "level3.round4": null,
          "level3.round5": null,
        ...
        navigate('/level3/round1'); // Redirect to the first round of the level
      } catch (err) {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-[#1a1f2e] to-[#111827] text-white flex items-center justify-center">Loading Level 3 Progress...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1f2e] to-[#111827] text-white p-4 sm:p-8 flex flex-col items-center justify-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 sm:gap-8 max-w-2xl mx-auto w-full mb-12">
        {renderRoundStone(1, "Sandwich Generation Squeeze")}
        {renderRoundStone(2, "Career Peak or Plateau")}
        {renderRoundStone(3, "Kids' Education Investment")}
        {renderRoundStone(4, "Health Reality & Insurance Strategy")}
        {renderRoundStone(5, "Retirement Acceleration or Lifestyle Upgrade")}
      </div>
