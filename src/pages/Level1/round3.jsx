...
const Round3 = () => {
  ...
  const nextDialogue = () => {
    if (currentDialogueIndex < mascotDialogues.length - 1) {
      ...
      if (showHospitalChoice) {
        ...
        setMascotDialogues([{ text: "Doctors inform: Baby needs NICU care for a few days.", audioSrc: null }, { text: "Extra Bill: ₹1,00,000", audioSrc: null }]);
        setCurrentDialogueIndex(0);
  ...
  const saveUserProgress = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        await updateDoc(userProgressRef2, { "level1.round3": userChoices });
      } else {
        await setDoc(userProgressRef, { "level1": { "round3": userChoices } });
      }
      if (docSnap2.exists()) {
        await updateDoc(userProgressRef, { "level":1, "round":3, "round3_completed": true });
      } else {
        await setDoc(userProgressRef, { "level":1, "round":3, "level1": { "round3": userChoices }, "round3_completed": true });
      }
  ...
  const handleHospitalSelection = (hospitalType) => {
    ...
    switch (hospitalType) {
      case 'Government Hospital':
        ...
        happiness = 'Medium';
        stressLevel = 'Low';
        hospitalDescription = 'Safe delivery, money mostly intact ✅';
        break;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1f2e] to-[#111827] text-white overflow-hidden">
      ...
      {(showHospitalChoice || showPaymentChoice || showComplicationTwist || showHospitalExtras || showGoingHome || showSummary) && (
              ...
              <div className="flex items-center gap-3 mb-3">
                <div className="flex-1 bg-[#374151] rounded-full h-3">
                  <div 
                    ...
                    style={{
                      width: showHospitalChoice ? '20%' :
                             showPaymentChoice ? '40%' :
                             showComplicationTwist ? '60%' :
                             showHospitalExtras ? '70%' :
                             showGoingHome ? '80%' :
                             showSummary ? '100%' : '0%'
                    }}
                  ></div>
                ...
                <span className="text-[#58cc02] font-bold text-sm">
                  {showHospitalChoice ? '1/5' :
                   showPaymentChoice ? '2/5' :
                   showComplicationTwist ? '3/5' :
                   showHospitalExtras ? '4/5' :
                   showGoingHome ? '5/5' : '5/5'}
                </span>
              </div>
              ...
              {showHospitalChoice && (
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <p className="text-gray-400 text-sm">Facilities: Basic, Happiness: Medium, Stress: Low (affordable), Outcome: Safe delivery, money mostly intact ✅</p>
                  </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-4">
                    <button 
                      className="w-full py-4 bg-gradient-to-r from-[#58cc02] to-[#2fa946] text-white rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2"
                      onClick={() => {
                        window.location.href = '/level1/round4';
                      }}
                    >
