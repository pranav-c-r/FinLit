...
const Level2Overview = () => {
    ...
    const fetchRoundProgress = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          console.log("No user progress found for Level 2, starting fresh.");
        }
      } catch (err) {
        console.error("Error fetching Level 2 round progress:", err);
        setError("Failed to load Level 2 progress. Please try again.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderRoundStone = (roundNumber, title) => {
    // For Level 2, Round 1 is unlocked if Level 1 Round 6 is completed
    const isUnlocked = roundNumber === 1 
      ? roundProgress.round1_completed // For round 1, just check if it's explicitly completed or implicitly available
      : roundProgress[`round${roundNumber - 1}_completed`];

    const isCurrent = roundNumber === 1 
      ? !roundProgress.round1_completed // Round 1 is current if not completed
      : (roundProgress[`round${roundNumber - 1}_completed`] && !roundProgress[`round${roundNumber}_completed`]);
    ...
    return (
      <Link 
        key={roundNumber}
        to={isUnlocked ? `/level2/round${roundNumber}` : '#'}
  ...
  const handleRestartLevel = async () => {
    ...
    if (window.confirm("Are you sure you want to restart Level 2? All your progress for this level will be reset.")) {
      try {
        const userProgressRef = doc(database, "Users", auth.currentUser.uid);
        await updateDoc(userProgressRef, {
          ...
          round5_completed: false,
          round6_completed: false,
          "level2.round1": null, // Reset round-specific data
          "level2.round2": null,
          "level2.round3": null,
          "level2.round4": null,
          "level2.round5": null,
        ...
        navigate('/level2/round1'); // Redirect to the first round of the level
      } catch (err) {
  ...
  const handleRestartGame = async () => {
    ...
    if (window.confirm("Are you sure you want to restart the entire game? All your progress will be lost.")) {
      try {
        ...
        navigate('/level1/round1'); // Redirect to the very first round of the game
      } catch (err) {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-[#1a1f2e] to-[#111827] text-white flex items-center justify-center">Loading Level 2 Progress...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1f2e] to-[#111827] text-white p-4 sm:p-8 flex flex-col items-center justify-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 sm:gap-8 max-w-2xl mx-auto w-full mb-12">
        {renderRoundStone(1, "Career Leap or Comfort Zone")}
        {renderRoundStone(2, "The Hospital Rush")}
        {renderRoundStone(3, "House vs. Freedom")}
        {renderRoundStone(4, "Side Hustle Temptation")}
        {renderRoundStone(5, "Health Reality Check")}
        {renderRoundStone(6, "Money Growth vs. Lifestyle Inflation")}
      </div>
